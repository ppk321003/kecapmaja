...
for idx, line in enumerate(lines):
    ...
    # Collect items yang termasuk dalam 2886
        ...
        if leading_semi >= 11:
            parts = line.rstrip('\n').split(';')
            periode_val = parse_number(parts[periode_ini_col].strip()) if periode_ini_col < len(parts) else None
...
if wa_2886_items:
    ...
    print(f"Match: {'✅' if abs(total_items - kegiatan_2886_aggregate) < 1 else '❌'}" if 'kegiatan_2886_aggregate' in locals() else "")
...
for idx, line in enumerate(lines):
    ...
    if re.search(r';GG\.2903;', line):
        ...
        print(f"✅ Kegiatan 2903 aggregate - Row {row_num}:")
        print(f"   Periode Ini: {parts[periode_ini_col].strip() if periode_ini_col < len(parts) else 'N/A'} = {periode_val:,.0f}" if periode_val else "")
...
for kegiatan in sorted(sheet_values.keys()):
    ...
    if kegiatan in csv_aggregate_values:
        ...
        status = "✅ Match" if abs(diff) < 1 else f"❌ Diff {diff:,.0f}"
