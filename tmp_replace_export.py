...
start_marker = 'const exportBAPPToDocx = async ('
end_marker = 'const getAnomalyPPLValue ='
start = text.find(start_marker)
end = text.find(end_marker, start)
if start == -1 or end == -1:
    raise SystemExit(f'Could not locate markers start={start} end={end}')
new_func = '''const exportBAPPToDocx = async (
  pmlRow: any,
