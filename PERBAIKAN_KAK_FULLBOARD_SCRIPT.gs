/**
 * Script Google Apps untuk memproses sheet "KerangkaAcuanKerja" dengan jenis paket meeting "Fullboard",
...
function KAKfullBoard() {
  ...
  crucialHeaders.forEach(header => {
    if (idx[header] === -1) {
      Logger.log(`Warning: Kolom krusial "${header}" tidak ditemukan di sheet. Beberapa fungsi mungkin tidak berjalan sesuai harapan.`);
    }
  });

  // ID dokumen template Google Docs
  const templateDocId = '1Dgeh6GwdZoc6MMfsyD25_JKuuPdX4kaBd-toX-ofSp8';
  ...
  // **FUNGSI FIXED UNTUK FORMAT TANGGAL**
  const formatTanggal = (dateValue) => {
    if (!dateValue) return '';
    
    try {
      // Handle berbagai tipe input tanggal
      let date;
      
      if (dateValue instanceof Date) {
        // Jika sudah berupa Date object, gunakan langsung
        date = new Date(dateValue);
      } else if (typeof dateValue === 'string' || typeof dateValue === 'number') {
        // Jika string atau number, parse sebagai tanggal
        date = new Date(dateValue);
      } else {
        return ''; // Tidak valid
      }
      ...
      // **PERBAIKAN UTAMA: Gunakan timezone Jakarta (WIB)**
      const options = {
      ...
      // Format dengan locale Indonesia dan timezone Jakarta
      return date.toLocaleDateString('id-ID', options);
    } catch (e) {
      Logger.log('Error formatting date: ' + e + ' for value: ' + dateValue);
      return '';
    }
  };
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    // Ambil nilai status, jenis KAK, dan jenis paket meeting dengan aman
    const jenisKAK = idx['Jenis Kerangka Acuan Kerja'] !== -1 ? row[idx['Jenis Kerangka Acuan Kerja']] || '' : '';

    // --- Pemeriksaan baris kosong ---
    if (idx['Jenis Kerangka Acuan Kerja'] !== -1 && (!jenisKAK || jenisKAK.toString().trim() === '')) {
      continue; // Lewati baris ini jika kolom Jenis KAK kosong
    }

    // Ambil nilai status dan jenis paket meeting setelah memastikan baris bukan baris kosong
    const status = idx['Status'] !== -1 ? row[idx['Status']] : null;
    const jenisPaketMeeting = idx['Jenis Paket Meeting'] !== -1 ? (row[idx['Jenis Paket Meeting']] || '').toString().trim() : 'Kolom "Jenis Paket Meeting" tidak ditemukan';

    // Periksa apakah jumlah kolom mencukupi sebelum mencoba mengakses data
    const maxIndex = Math.max(...Object.values(idx).filter(index => index !== -1));
    if (row.length <= maxIndex) {
      Logger.log(`Melewati Baris ${i+1} - Jumlah kolom (${row.length}) tidak mencukupi.`);
      continue;
    }

    // --- Kondisi utama untuk memproses baris ---
    // Hanya proses jika status kosong (atau spasi kosong) DAN Jenis Kerangka Acuan Kerja adalah 'Belanja Paket Meeting'
    // DAN Kolom 'Jenis Paket Meeting' ada DAN nilainya adalah 'Fullboard'
    if ((!status || status.toString().trim() === '') &&
      (jenisKAK === 'Belanja Paket Meeting') &&
      (idx['Jenis Paket Meeting'] !== -1 && jenisPaketMeeting === 'Fullboard')) {

      Logger.log(`Memproses Baris ${i+1} - Kondisi terpenuhi.`);
      ...
      // Ambil Jumlah Gelombang, pastikan itu angka
      ...
      for (let k = 1; k <= 15; k++) {
        ...
        if (idx[colName] !== -1) {
          ...
          break; // Berhenti jika kolom Nama Kegiatan-k tidak ditemukan
        }
      }
      // Jumlah kegiatan yang benar-benar terisi
      ...
      // Gabung nomor hanya untuk kegiatan yang terisi
      ...
      // --- Modifikasi loop ini agar hanya berjalan sebanyak numKegiatan ---
      for (let k = 1; k <= numKegiatan; k++) {
        const volCol = 'Volume-' + k;
        const satCol = 'Satuan-' + k;
        const hargaCol = 'Harga Satuan-' + k;

        // Karena kita hanya looping sampai numKegiatan, kita tahu kegiatanList[k-1] ada
        // Tapi tetap cek idx untuk keamanan jika kolom tidak ada sama sekali
      ...
      // --- Logika Pembentukan String Tanggal Pelaksanaan Gelombang (Diperbaiki untuk format bullet) ---
      let tanggalGelombangString = '';
      let gelombangParts = [];

      // Pastikan jmlGelombang adalah angka dan lebih dari 0
      if (jmlGelombang > 0) {
        for (let g = 1; g <= jmlGelombang; g++) {
          ...
          // **PERBAIKAN: Gunakan fungsi formatTanggal yang sudah diperbaiki**
          ...
          // Tambahkan tanggal hanya jika salah satu tanggal ada
          if (tglMulaiGFormatted || tglAkhirGFormatted) {
             ...
             if (tglMulaiGFormatted && tglAkhirGFormatted) {
                 gelombangPart += ` sampai dengan`;
                  ...
                  gelombangPart += ` sampai dengan`; // Jika tglMulai kosong tapi tglAkhir ada
             }
          ...
          // Jika kedua tanggal kosong, string akan tetap "Gelombang X"

          gelombangParts.push(gelombangPart);
        }

        // Format string berdasarkan jumlah gelombang
        if (gelombangParts.length === 1) {
          tanggalGelombangString = gelombangParts[0]; // Satu gelombang, tetap satu baris
        } else if (gelombangParts.length > 1) {
          // Lebih dari satu gelombang, format sebagai daftar dengan bullet point
          tanggalGelombangString = gelombangParts.map(part => `- ${part}`).join('\n');
        }
        // Jika gelombangParts kosong (jmlGelombang <= 0 atau data tidak ada), tanggalGelombangString tetap kosong
      }
      // --- Akhir Logika Pembentukan String Tanggal Pelaksanaan Gelombang ---

      // **PERBAIKAN TANGGAL: Gunakan fungsi formatTanggal yang sudah diperbaiki**
      ...
      // Tambahkan logging untuk debugging tanggal
      Logger.log(`--- Baris ${i+1} ---`);
      Logger.log(`Tanggal Mulai (raw): ${tglMulai} -> (formatted): ${tglMulaiFormatted}`);
      Logger.log(`Tanggal Akhir (raw): ${tglAkhir} -> (formatted): ${tglAkhirFormatted}`);
      Logger.log(`Tanggal Pengajuan (raw): ${tglPengajuan} -> (formatted): ${tglPengajuanFormatted}`);
      Logger.log(`Jumlah Gelombang: ${jmlGelombang}`);
      Logger.log(`Tanggal Pelaksanaan Gelombang String: "${tanggalGelombangString}"`);
      ...
      body.replaceText('<<Jumlah Gelombang>>', jmlGelombangStr); // Tetap gunakan string aslinya jika perlu, atau jmlGelombang jika hanya angka
      ...
      // Ganti placeholder Tanggal Pelaksanaan Gelombang
      ...
      try {
        ...
        Logger.log(`Baris ${i+1} berhasil diproses.`);
      } catch (e) {
        Logger.log(`Error updating sheet for row ${i + 1}: ${e}`);
      }

      Utilities.sleep(500); // Tetap pertahankan jeda untuk menghindari masalah kuota API
    }
  }
  Logger.log("--- Proses Selesai ---");
  try {
    Logger.log("Menjalankan fungsi kirim WA otomatis...");
    sendDataFromKAKToWA_KECAP2();
 ...
 */
function sendDataFromKAKToWA_KECAP2() {
  try {
    // Sheet sumber
