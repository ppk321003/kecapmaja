...
const getTotalAnggotaKeluargaValue = (row: any, defaultValue: any = "-") => {
  const rawRow = row && Array.isArray(row.__rawRow) ? row.__rawRow : undefined;
  if (rawRow && rawRow.length > 8 && rawRow[8] !== "" && rawRow[8] !== undefined && rawRow[8] !== null) {
    return rawRow[8];
  }
  return getColumnValue(row, "total_anggota_keluarga", ["total anggota keluarga"], defaultValue);
};
...
export default function MonitoringLapanganKualitasTab({ spreadsheetId }: Props) {
  ...
  const { data: anggotaData = [], loading: l4, error: anggotaError } = useGoogleSheetsData({ spreadsheetId, sheetName: "ANGGOTA KELUARGA", refreshKey: qualityReloadCounter });
  ...
  const { data: anggotaHeader = [], loading: h3, error: anggotaHeaderError } = useGoogleSheetsData({ spreadsheetId, sheetName: "ANGGOTA KELUARGA", range: "ANGGOTA KELUARGA!A2", mode: "single-cell", refreshKey: qualityReloadCounter });
  ...
  const anggotaTotals = useMemo(() => {
    ...
    sortedAnggotaRows.forEach((r: any) => {
      ...
      totals.totalAnggotaKeluarga += sumNumeric(getTotalAnggotaKeluargaValue(r, "0"));
    });
    return totals;
  }, [sortedAnggotaRows]);
  ...
  const exportExcel = (tab: string) => {
    ...
    const rows = baseRows.map((row: any) => {
      ...
      return [
        ...
        getTotalAnggotaKeluargaValue(row, "0"),
      ];
    });
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-slate-50 sticky top-0 z-20">
                  ...
                  <TableHead className="whitespace-normal break-words cursor-pointer text-center max-w-[150px]" onClick={() => toggleSort("anggota", "tinggal_bersama_keluarga")}>Tinggal Bersama Keluarga{renderSortIcon(sortByAnggota === "tinggal_bersama_keluarga", sortOrderAnggota)}</TableHead>
                  <TableHead className="whitespace-normal break-words cursor-pointer text-center max-w-[150px]" onClick={() => toggleSort("anggota", "anggota_keluarga_baru")}>Anggota Keluarga Baru{renderSortIcon(sortByAnggota === "anggota_keluarga_baru", sortOrderAnggota)}</TableHead>
                  <TableHead className="whitespace-normal break-words cursor-pointer text-center max-w-[120px]" onClick={() => toggleSort("anggota", "meninggal")}>Meninggal{renderSortIcon(sortByAnggota === "meninggal", sortOrderAnggota)}</TableHead>
                  <TableHead className="whitespace-normal break-words cursor-pointer text-center max-w-[150px]" onClick={() => toggleSort("anggota", "pindah_dn")}>Pindah Dalam Negeri (DN){renderSortIcon(sortByAnggota === "pindah_dn", sortOrderAnggota)}</TableHead>
                  <TableHead className="whitespace-normal break-words cursor-pointer text-center max-w-[150px]" onClick={() => toggleSort("anggota", "pindah_ln")}>Pindah Luar Negeri (LN){renderSortIcon(sortByAnggota === "pindah_ln", sortOrderAnggota)}</TableHead>
                  <TableHead className="whitespace-normal break-words cursor-pointer text-center max-w-[140px]" onClick={() => toggleSort("anggota", "tidak_ditemukan")}>Tidak Ditemukan{renderSortIcon(sortByAnggota === "tidak_ditemukan", sortOrderAnggota)}</TableHead>
                  <TableHead className="whitespace-normal break-words cursor-pointer text-center max-w-[160px]" onClick={() => toggleSort("anggota", "anggota_keluarga_khusus")}>Anggota Keluarga Khusus{renderSortIcon(sortByAnggota === "anggota_keluarga_khusus", sortOrderAnggota)}</TableHead>
                  <TableHead className="whitespace-normal break-words cursor-pointer text-center max-w-[160px]" onClick={() => toggleSort("anggota", "total_anggota_keluarga")}>Total Anggota Keluarga{renderSortIcon(sortByAnggota === "total_anggota_keluarga", sortOrderAnggota)}</TableHead>
                  {showPplPmlColumns ? (
                    <>
              ...
              <TableBody>
                {sortedAnggotaRows.map((r: any, idx: number) => (
                  <TableRow key={idx} className="even:bg-white">
                    ...
                    <TableCell className="text-center">{formatNumberValue(getTotalAnggotaKeluargaValue(r, "-"))}</TableCell>
                    {showPplPmlColumns ? (
                      <>
                        <TableCell className="text-center">{getNamaPplFromRow(r)}</TableCell>
                        <TableCell className="text-center">{formatPersonNameValue(getColumnValue(r, "nama_pml", ["nama_pml", "nama pml", "pml"], "-"))}</TableCell>
