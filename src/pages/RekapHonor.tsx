          ...
          {honorData.map((item) => (
            ...
            >
              ...
              {isLoading ? (
                ...
                <div className="p-4 bg-red-100 rounded-lg text-red-600 text-center">
                  Gagal memuat data. Silakan coba lagi.
                </div>
              ) : data && data.length > 0 ? (
                // Jika Anda ingin menampilkan data dalam bentuk list, bisa buat di sini
                <div className="mt-4 p-4 bg-white border border-gray-200 rounded-lg shadow-inner max-h-96 overflow-y-auto">
                  {/* Contoh tampilan data sebagai list */}
                  {data.map((item, index) => (
                    <div
                      key={index}
                      className="p-2 border-b last:border-b-0 hover:bg-gray-100 cursor-pointer rounded"
                    >
                      <pre className="text-sm font-mono">{JSON.stringify(item, null, 2)}</pre>
                    </div>
                  ))}
                </div>
              ) : (
