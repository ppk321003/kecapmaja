  ...
  return <Layout>
      <div className="space-y-6">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList>
            {honorData.map(item => <TabsTrigger key={item.id} value={item.id}>
                {item.title}
              </TabsTrigger>)}
          </TabsList>
          
          {honorData.map(item => <TabsContent key={item.id} value={item.id}>
              <Card>
                ...
                <CardContent>
                  {isLoading ? <div className="space-y-2">
                    ...
                    </div> : data && data.length > 0 ? <DataTable title={item.title} columns={generateColumns(data)} data={data} /> : <p className="text-center py-8">Tidak ada data yang tersedia.</p>}
                </CardContent>
              </Card>
            </TabsContent>)}
        </Tabs>
