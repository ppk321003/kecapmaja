...
export default function CekSBML() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [tugasResult, masterResult] = await Promise.all([supabase.functions.invoke("google-sheets", {
        body: {
  ...
  const sbmlBadgeContent = useMemo(() => {
    ...
    return <div className="flex flex-col gap-1 ml-auto">
      ...
      <Card className="border-l-4 border-l-blue-500">
        ...
        <CardContent className="pt-0">
          <div className="flex gap-3 items-center flex-wrap">
            ...
            {sbmlBadgeContent && <div className="ml-auto">
                {sbmlBadgeContent}
              </div>}
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            {data.length > 0 && <Badge variant="secondary" className="text-sm">
        ...
        <CardContent>
          {loading ? <div className="text-center py-8">
            ...
            </div> : data.length === 0 ? <div className="text-center py-8">
              ...
              <Table className="min-w-full">
                  ...
                  <TableRow>
                    ...
                    <TableHead className="min-w-[160px]">
                      <button className="flex items-center gap-1 hover:bg-gray-100 px-2 py-1 rounded transition-colors" onClick={() => handleSort('namaMitra')}>
                        ...
                        {sortField === 'namaMitra' && <span className="text-xs">
                            {sortDirection === 'asc' ? '↑' : '↓'}
                          </span>}
                      </button>
                    </TableHead>
                    ...
                    <TableHead className="text-right min-w-[120px]">
                      <button className="flex items-center gap-1 hover:bg-gray-100 px-2 py-1 rounded transition-colors ml-auto" onClick={() => handleSort('jumlah')}>
                        Jumlah
                        <ArrowUpDown className="h-4 w-4" />
                        {sortField === 'jumlah' && <span className="text-xs">
                            {sortDirection === 'asc' ? '↑' : '↓'}
                          </span>}
                      </button>
                    </TableHead>
                ...
                <TableBody>
                  {sortedData.map((row, index) => <TableRow key={`${row.namaMitra}_${row.nik}`} className={row.isExceeded ? "bg-red-50" : ""}>
                      ...
                      <TableCell className="px-4 min-w-[150px]">
                        <div className="flex justify-end">
                          <Input type="text" value={row.pekerjaanProvinsi === 0 ? "" : row.pekerjaanProvinsi.toLocaleString('id-ID')} onChange={e => handlePekerjaanProvinsiChange(index, e.target.value)} className="text-right w-32 h-9 text-sm font-medium border-gray-300 focus:border-blue-500" placeholder="0" />
                        </div>
                      </TableCell>
