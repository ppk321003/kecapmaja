...
export default function EntriPetugas() {
  ...
  const handleSort = (field: keyof Petugas) => {
    if (field === sortField) setSortAsc(!sortAsc);else {
      setSortField(field);
      setSortAsc(true);
    }
  };
  ...
  return <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-red-500">Entri Petugas</h1>
          ...
          <DialogContent>
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-2">
                ...
                {Object.keys(petugasSchema.shape).map(key => key === "kecamatan" ? <FormField key={key} control={form.control} name="kecamatan" render={({
                field
              }) => <FormItem>
                          ...
                          <FormMessage />
                        </FormItem>} /> : <FormField key={key} control={form.control} name={key as keyof PetugasFormData} render={({
                field
              }) => <FormItem>
                          <FormLabel>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Input placeholder="Cari petugas..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="max-w-xs" />
          </div>
        </CardHeader>

        <CardContent>
          {loading ? <p className="text-center py-6 text-muted-foreground">
              Memuat data...
            </p> : <>
              <Table className="text-sm">
                <TableHeader>
                  <TableRow className="h-auto">
                    {["no", "nik", "nama", "pekerjaan", "alamat", "bank", "rekening", "kecamatan"].map(col => <TableHead key={col} onClick={() => handleSort(col as keyof Petugas)} className="cursor-pointer select-none py-1">
                ...
                <TableBody>
                  {paginatedPetugas.map(p => <TableRow key={p.rowIndex} className="h-auto">
              ...
              <div className="flex justify-between items-center mt-3 text-sm">
                ...
                <div className="flex gap-2">
                  <Button disabled={page === 1} onClick={() => setPage(1)} variant="outline">
                    Awal
                  </Button>
                  ...
                  <Button disabled={page === totalPages} onClick={() => setPage(totalPages)} variant="outline">
                    Akhir
                  </Button>
                </div>
              </div>
