...
const WA_SPK_SPREADSHEET_ID = "1sQPukgDuGxku_cQ8bsYc4gSo2KdOIanyC4VJYdn2lJ0";
...
export default function RekapSPKBAST() {
    ...
    try {
      ...
      for (let i = 1; i < tugasRows.length; i++) {
        ...
        if (periode === cleanedPeriodeFilter && namaPetugas && hargaSatuan && realisasi) {
          matchCount++;
          const processedPetugas = processPetugasData(
            namaPetugas, nikPetugas, hargaSatuan, realisasi, satuan, 
            statusTTD, statusNotif, masterPetugas, i, namaKegiatan, periode, role
          );
      ...
      for (const petugas of petugasTugas) {
        ...
        existing.allMappings.push({
          ...
          statusNotif: petugas.statusNotif,
        ...
        if (petugas.statusNotif === "Sudah dikirim") {
          existing.statusNotif = "Sudah dikirim";
        }
      }
  ...
  }, [filterBulan, filterTahun, cleanPeriode, processPetugasData, toast, callEdgeFunction, sbmlData, validateRow, isPPK, syncToWaSpkSheet]);
  ...
  const handleStatusChange = useCallback(async (namaMitra: string, nik: string, newStatus: string) => {
    ...
    try {
      ...
      console.log("🔄 UPDATE STATUS TTD REQUEST:");
      ...
      if (successCount > 0) {
        ...
        // Sync ke WA-SPK sheet setelah perubahan status
        ...
        await syncToWaSpkSheet(updatedData);
  ...
  }, [data, isPPK, filterBulan, filterTahun, cleanPeriode, toast, callEdgeFunction, fetchData, syncToWaSpkSheet]);
    ...
    try {
      ...
      for (const mapping of relevantMappings) {
        try {
          ...
          // Update hanya kolom status notif (kolom Y, index 24)
          const updatedRow = [...currentRow];
      ...
      if (successCount > 0) {
        toast({
          title: "Berhasil",
          description: `Notifikasi ${item.namaMitra} diubah menjadi "${newStatus}" (${successCount}/${relevantMappings.length} data terupdate)`
        });

        // Sync ke WA-SPK sheet setelah perubahan status notif
        const updatedData = data.map(row => 
          row.namaMitra === namaMitra && row.nik === nik ? {
            ...row,
            statusNotif: newStatus
  ...
  const handleManualSyncToWaSpk = useCallback(async () => {
    if (!isPPK) {
      toast({
        title: "Akses Ditolak",
        description: "Hanya PPK yang dapat melakukan sync",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Gagal menyinkronisasi data: " + error.message,
        variant: "destructive"
      });
    } finally {
      setLoading(false);
              ...
              <Table className="min-w-full">
                <TableHeader>
                  <TableRow>
                    ...
                    {isPPK && (
                      <TableHead className="w-48 text-center">Notif Mitra</TableHead>
                    )}
                ...
                <TableBody>
                      ...
                      {isPPK && (
                        <TableCell className="text-center">
                          <div className="flex flex-col items-center gap-3">
                            <Badge 
                              variant={row.statusNotif === "Sudah dikirim" ? "default" : "destructive"} 
                              className={`text-xs px-3 py-1 ${
                                row.statusNotif === "Sudah dikirim" ? 
                            ...
                            >
                              {row.statusNotif === "Sudah dikirim" ? (
                                ...
                                <BellOff className="h-3 w-3 mr-1 inline" />
                              )}
                              {row.statusNotif}
                            </Badge>
                            ...
                            <div className="flex items-center space-x-2">
                              <Switch 
                                checked={row.statusNotif === "Sudah dikirim"} 
                                onCheckedChange={checked => 
                                  handleNotifChange(
                                    row.namaMitra, 
                                    row.nik, 
                                    checked ? "Sudah dikirim" : "Belum dikirim"
                                  )
                                } 
                                className="data-[state=checked]:bg-green-600" 
                              />
                              <Label className="text-xs">
                                {row.statusNotif === "Sudah dikirim" ? "Batalkan" : "Kirim"}
                              </Label>
                            </div>
                          </div>
                        </TableCell>
                  ...
                  {totals && (
                    <TableRow className="bg-gray-50 font-semibold border-t-2 border-gray-300">
                      ...
                      <TableCell></TableCell>
                      {/* Kolom kosong untuk Notif Mitra jika PPK */}
