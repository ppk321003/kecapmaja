...
class AngkaKreditCalculator {
  ...
  static getKebutuhanPangkat(golonganSekarang: string, kategori: string): number {
    const kebutuhanKeahlian: {
      ...
      'III/b': 50,
      // Ahli Pertama III/b → III/c
      'III/c': 100,
      // Ahli Muda III/c → III/d
      'III/d': 100,
      // Ahli Muda III/d → IV/a
      'IV/a': 150,
      // Ahli Madya IV/a → IV/b
      'IV/b': 150,
      // Ahli Madya IV/b → IV/c
      'IV/c': 150,
      // Ahli Madya IV/c → IV/d
...
const ProgressBar: React.FC<{
  ...
  return <div className="mb-6 p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className={`text-xs font-medium px-2 py-1 rounded ${isTidakAda ? 'bg-gray-100 text-gray-800' : bisaUsul ? 'bg-green-100 text-green-800' : bulanDibutuhkan <= 6 ? 'bg-green-100 text-green-800' : bulanDibutuhkan <= 12 ? 'bg-blue-100 text-blue-800' : bulanDibutuhkan <= 24 ? 'bg-yellow-100 text-yellow-800' : 'bg-orange-100 text-orange-800'}`}>
          {getStatusText()}
        </div>
      </div>
      
      {!isTidakAda && penjelasan && <div className="mb-3 p-2 bg-blue-50 rounded border border-blue-200">
      ...
      {!isTidakAda && <>
          ...
          <div className="w-full bg-gray-200 rounded-full h-3 mb-2">
            <div className={`bg-gradient-to-r ${getColorClass()} h-3 rounded-full transition-all duration-500 ease-out`} style={{
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-2 text-xs text-gray-600 mb-3">
        ...
        {!isTidakAda && <div className="text-center">
      ...
      {bisaUsul && !isTidakAda && <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded text-center">
          <span className="text-green-800 text-sm font-semibold">
            {isKenaikanJenjang && type === 'jabatan' ? '✅ Bisa mengusulkan kenaikan JABATAN dan PANGKAT sekaligus!' : '✅ Sudah memenuhi syarat untuk diusulkan!'}
          </span>
        </div>}
    </div>;
};
...
const BiodataSederhana: React.FC<{
  karyawan: Karyawan;
  akRealSaatIni: number;
  akTambahan: number;
}> = ({
...
const PredikatKinerjaRadio: React.FC<{
  ...
  return <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {predikatOptions.map(option => <div key={option.value} className={`relative rounded-lg border-2 p-3 cursor-pointer transition-all duration-200 ${selectedValue === option.value ? 'border-blue-500 bg-blue-50 shadow-md' : 'border-gray-200 bg-white hover:border-gray-300'}`} onClick={() => onValueChange(option.value)}>
            <div className="flex items-center mb-2">
              <div className={`w-4 h-4 rounded-full border-2 mr-2 flex items-center justify-center ${selectedValue === option.value ? 'border-blue-500 bg-blue-500' : 'border-gray-400'}`}>
...
const EmployeeTable: React.FC<{
  ...
  if (loading) return <div className="bg-white rounded-xl p-4 shadow-lg border border-gray-200 mb-6">
  ...
  return <div className="bg-white rounded-xl p-4 shadow-lg border border-gray-200 mb-6">
      ...
      {filteredKaryawan.length === 0 ? <div className="text-center py-8"><div className="text-gray-400 text-4xl mb-2">🔍</div><h3 className="text-sm font-semibold text-gray-600 mb-1">Tidak ada karyawan ditemukan</h3><p className="text-gray-500 text-xs">Coba ubah kata kunci pencarian</p></div> : <div className="overflow-x-auto">
          <table className="w-full text-sm text-left text-gray-700">
            <thead className="text-xs text-gray-700 uppercase bg-gray-50 border-b">
              <tr>
                {['nama', 'nip', 'golongan', 'jabatan', 'akKumulatif'].map(field => <th key={field} className="px-3 py-2 cursor-pointer hover:bg-gray-100" onClick={() => handleSort(field as keyof Karyawan)}>
            ...
            <tbody>
              {sortedKaryawan.map(karyawan => <tr key={karyawan.nip} className={`border-b hover:bg-gray-50 transition-colors ${selectedNip === karyawan.nip ? 'bg-blue-50' : ''}`}>
...
const EstimasiKenaikan: React.FC<{
  karyawan: Karyawan;
}> = ({
  karyawan
}) => {
  ...
  return <div className="bg-white rounded-xl p-4 shadow-lg border border-gray-200">
      ...
      {estimasi.isKenaikanJenjang && <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
      ...
      {estimasi.isKenaikanJenjang && estimasi.bisaUsulJabatan && <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
...
const InputKinerjaForm: React.FC<{
  ...
  return <div className="bg-white rounded-xl p-4 shadow-lg border border-gray-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
        <div className="space-y-2"><label className="text-sm font-medium text-gray-700">Periode Penilaian</label><input type="month" value={formData.periode} onChange={e => setFormData({
          ...formData,
          periode: e.target.value
        })} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" /></div>
        <div className="space-y-2"><label className="text-sm font-medium text-gray-700">Jenis Penilaian</label><select value={formData.jenisPenilaian} onChange={e => setFormData({
          ...formData,
          jenisPenilaian: e.target.value as 'Tahunan' | 'Periodik'
        })} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"><option value="Tahunan">Tahunan</option><option value="Periodik">Periodik</option></select></div>
        {formData.jenisPenilaian === 'Periodik' && <div className="space-y-2"><label className="text-sm font-medium text-gray-700">Bulan Periodik</label><input type="number" min="1" max="11" value={formData.bulanPeriodik} onChange={e => setFormData({
          ...formData,
          bulanPeriodik: parseInt(e.target.value)
        })} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" /></div>}
      </div>
      <div className="mb-4"><PredikatKinerjaRadio selectedValue={formData.predikatKinerja} onValueChange={value => setFormData({
        ...formData,
        predikatKinerja: value
      })} /></div>
      <div className="space-y-2 mb-4"><label className="text-sm font-medium text-gray-700">Keterangan</label><input type="text" value={formData.keterangan} onChange={e => setFormData({
        ...formData,
        keterangan: e.target.value
      })} placeholder="Capaian SKP, evaluasi, dll." className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" /></div>
...
const App: React.FC = () => {
  ...
  const handleSaveInput = (newInput: InputKinerja) => {
    ...
    if (selectedKaryawan) {
      const updatedKaryawanList = karyawanList.map(k => k.nip === selectedKaryawan.nip ? {
        ...k,
        akKumulatif: k.akKumulatif + newInput.akDiperoleh
      } : k);
