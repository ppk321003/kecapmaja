// App.tsx - VERSI DIPERBAIKI DENGAN FIX LOGIKA PENGHITUNGAN SESUAI KEBUTUHAN
...
class AngkaKreditCalculator {
  ...
  // Kebutuhan AK KUMLULATIF untuk kenaikan jabatan - DIPERBAIKI SESUAI INPUT
  ...
  static hitungEstimasiKenaikan(karyawan: Karyawan, predikatAsumsi: number = 1.00): EstimasiKenaikan {
    ...
    const koefisien = this.getKoefisien(karyawan.jabatan);
    const akPerBulan = Number(((predikatAsumsi * koefisien) / 12).toFixed(2));

    const kebutuhanAKPangkat = this.getKebutuhanPangkat(karyawan.golongan, karyawan.kategori);
    const kebutuhanAKJabatan = this.getKebutuhanJabatan(karyawan.jabatan, karyawan.kategori);

    const kekuranganAKPangkat = Math.max(0, kebutuhanAKPangkat - akRealSaatIni);
    const kekuranganAKJabatan = Math.max(0, kebutuhanAKJabatan - akRealSaatIni);

    const bulanDibutuhkanPangkat = kekuranganAKPangkat > 0 ? Math.ceil(kekuranganAKPangkat / akPerBulan) : 0;
    const bulanDibutuhkanJabatan = kekuranganAKJabatan > 0 ? Math.ceil(kekuranganAKJabatan / akPerBulan) : 0;
    ...
    return {
      ...
      estimasiTanggalPangkat: estimasiTanggalPangkat.toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }),
      estimasiTanggalJabatan: estimasiTanggalJabatan.toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }),
      bisaUsulPangkat: kekuranganAKPangkat <= 0,
      bisaUsulJabatan: kekuranganAKJabatan <= 0,
      golonganBerikutnya,
      jabatanBerikutnya,
      akPerBulan,
      akRealSaatIni,
  ...
  // Helper: Golongan berikutnya
  static getGolonganBerikutnya(golongan: string, kategori: 'Keahlian' | 'Keterampilan'): string {
    const mapKeahlian: { [key: string]: string } = {
      'III/a': 'III/b', 'III/b': 'III/c', 'III/c': 'III/d', 'III/d': 'IV/a',
      'IV/a': 'IV/b', 'IV/b': 'IV/c', 'IV/c': 'IV/d', 'IV/d': 'IV/e'
    };
    const mapKeterampilan: { [key: string]: string } = {
      'II/a': 'II/b', 'II/b': 'II/c', 'II/c': 'II/d', 'II/d': 'III/a',
      'III/a': 'III/b', 'III/b': 'III/c', 'III/c': 'III/d'
    };
    const map = kategori === 'Keahlian' ? mapKeahlian : mapKeterampilan;
    return map[golongan] || 'Tidak Ada';
  }

  // Helper: Jabatan berikutnya
  static getJabatanBerikutnya(jabatan: string, kategori: 'Keahlian' | 'Keterampilan'): string {
    if (kategori === 'Keahlian') {
      if (jabatan.includes('Pertama')) return 'Ahli Muda';
      if (jabatan.includes('Muda')) return 'Ahli Madya';
      if (jabatan.includes('Madya')) return 'Ahli Utama';
    } else {
      if (jabatan.includes('Terampil')) return 'Mahir';
      if (jabatan.includes('Mahir')) return 'Penyelia';
    }
    return 'Tidak Ada';
  }

  // Helper: Penjelasan kebutuhan kumulatif
  static getPenjelasanKebutuhan(jabatan: string, kategori: 'Keahlian' | 'Keterampilan'): string | null {
    if (kategori === 'Keahlian') {
      if (jabatan.includes('Muda')) return 'Total 200 AK kumulatif (100 dari Ahli Pertama + 100 dari Ahli Muda) untuk naik ke Ahli Madya';
      if (jabatan.includes('Madya')) return 'Total 450 AK kumulatif (100 + 100 + 150 + 150 + 150) untuk naik ke Ahli Utama';
    } else {
      if (jabatan.includes('Terampil')) return 'Total 60 AK kumulatif (20 + 20 + 20 dari II/b, II/c, II/d) untuk naik ke Mahir';
      if (jabatan.includes('Mahir')) return 'Total 100 AK kumulatif (50 + 50 dari III/a dan III/b) untuk naik ke Penyelia';
    }
    return null;
  }

  // Helper: Rekomendasi karir
  static getRekomendasiKarir(karyawan: Karyawan): string | null {
    const estimasi = this.hitungEstimasiKenaikan(karyawan);
    if (estimasi.kekuranganAKJabatan > 0 && estimasi.kekuranganAKJabatan <= 50) {
      return 'Fokus pada pengembangan kinerja dan pelatihan untuk mempercepat kenaikan jabatan.';
    }
    if (estimasi.akRealSaatIni < 50) {
      return 'Perlu peningkatan kinerja signifikan untuk mencapai jenjang berikutnya.';
    }
    return null;
  }
}

// ==================== KOMPONEN ESTIMASI DIPERBAIKI ===
const EstimasiKenaikan: React.FC<{ karyawan: Karyawan }> = ({ karyawan }) => {
  ...
  const kebutuhanPangkatAktual = isJenjangPertama ? estimasi.kebutuhanAKJabatan : estimasi.kebutuhanAKPangkat;
  const kekuranganPangkatAktual = Math.max(0, kebutuhanPangkatAktual - estimasi.akRealSaatIni);
  const bulanDibutuhkanPangkatAktual = kekuranganPangkatAktual > 0 ? Math.ceil(kekuranganPangkatAktual / estimasi.akPerBulan) : 0;

  const formatEstimasiWaktu = (bulan: number) => {
    if (bulan <= 0) return { tahun: 0, bulan: 0, formatted: '0 bulan' };
    const tahun = Math.floor(bulan / 12);
    const bulanSisa = bulan % 12;
    let formatted = '';
    if (tahun > 0 && bulanSisa > 0) formatted = `${tahun} tahun ${bulanSisa} bulan`;
    else if (tahun > 0) formatted = `${tahun} tahun`;
    else formatted = `${bulanSisa} bulan`;
    return { tahun, bulan: bulanSisa, formatted };
  };
      ...
      {isJenjangPertama && (
        <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
          <div className="flex items-start">
            <span className="text-yellow-500 mr-2">Info</span>
            <div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-gray-600">{isJenjangPertama ? 'Jenjang berikutnya:' : 'Pangkat berikutnya:'}</span>
              <span className="font-semibold">{isJenjangPertama ? 'Ahli Muda' : estimasi.golonganBerikutnya}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-semibold text-blue-600 text-xs">
                {estimasiTanggalPangkatAktual.toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}
              </span>
            </div>
          </div>
        </div>
...
// ==================== SEMUA KOMPONEN LAIN TETAP SAMA (tidak diubah tampilan) ====================
// [ParseNIP, BiodataSederhana, PredikatKinerjaRadio, EmployeeTable, InputKinerjaForm, EmployeeDashboard, App]
// ... (sama persis dengan aslinya, hanya logika di dalam class AngkaKreditCalculator yang diperbaiki)

const parseNIP = (nip: string) => {
  if (!nip || nip.length < 15) {
    return { tanggalLahir: '', tahunMasuk: '', jenisKelamin: 'L' };
  }
    ...
    <div className="bg-white rounded-xl p-4 shadow-lg border border-gray-200 mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="space-y-2">
          ...
          <div><p className="text-xs text-gray-500">AK Real Saat Ini</p><p className="font-bold text-blue-600 text-lg">{akRealSaatIni.toFixed(2)}</p>
            <p className="text-xs text-gray-500">(Database: {karyawan.akKumulatif.toFixed(2)} + Tambahan: {akTambahan.toFixed(2)})</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {predikatOptions.map((option) => (
          <div key={option.value} className={`relative rounded-lg border-2 p-3 cursor-pointer transition-all duration-200 ${selectedValue === option.value ? 'border-blue-500 bg-blue-50 shadow-md' : 'border-gray-200 bg-white hover:border-gray-300'}`} onClick={() => onValueChange(option.value)}>
            <div className="flex items-center mb-2">
              ...
              <span className={`font-semibold text-sm ${selectedValue === option.value ? 'text-blue-800' : 'text-gray-800'}`}>{option.label}</span>
            </div>
            <div className={`text-xs font-bold mt-1 ${selectedValue === option.value ? 'text-blue-800' : 'text-gray-700'}`}>{option.value * 100}%</div>
          </div>
        ))}
      </div>
    </div>
    ...
    return (
      <div className="bg-white rounded-xl p-4 shadow-lg border border-gray-200 mb-6">
        <div className="text-center py-8">
          <div className="text-gray-400 text-4xl mb-2">Loading</div>
  ...
  return (
    <div className="bg-white rounded-xl p-4 shadow-lg border border-gray-200 mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
        ...
        <div>
          <select value={filterKategori} onChange={(e) => setFilterKategori(e.target.value as any)} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
      ...
      {filteredKaryawan.length === 0 ? (
        <div className="text-center py-8">
          <div className="text-gray-400 text-4xl mb-2">Search</div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left text-gray-700">
            <thead className="text-xs text-gray-700 uppercase bg-gray-50 border-b">
              <tr>
                <th className="px-3 py-2 cursor-pointer hover:bg-gray-100" onClick={() => handleSort('nama')}><div className="flex items-center">Nama<span className="ml-1 text-xs">{getSortIcon('nama')}</span></div></th>
                <th className="px-3 py-2 cursor-pointer hover:bg-gray-100" onClick={() => handleSort('nip')}><div className="flex items-center">NIP<span className="ml-1 text-xs">{getSortIcon('nip')}</span></div></th>
                <th className="px-3 py-2 cursor-pointer hover:bg-gray-100" onClick={() => handleSort('golongan')}><div className="flex items-center">Golongan<span className="ml-1 text-xs">{getSortIcon('golongan')}</span></div></th>
                <th className="px-3 py-2 cursor-pointer hover:bg-gray-100" onClick={() => handleSort('jabatan')}><div className="flex items-center">Jabatan<span className="ml-1 text-xs">{getSortIcon('jabatan')}</span></div></th>
                <th className="px-3 py-2 cursor-pointer hover:bg-gray-100" onClick={() => handleSort('akKumulatif')}><div className="flex items-center">AK Awal<span className="ml-1 text-xs">{getSortIcon('akKumulatif')}</span></div></th>
                <th className="px-3 py-2 cursor-pointer hover:bg-gray-100"><div className="flex items-center">AK Tambahan</div></th>
                <th className="px-3 py-2 cursor-pointer hover:bg-gray-100"><div className="flex items-center">AK Real</div></th>
              ...
              {sortedKaryawan.map((karyawan) => (
                <tr key={karyawan.nip} className={`border-b hover:bg-gray-50 transition-colors ${selectedNip === karyawan.nip ? 'bg-blue-50' : ''}`}>
                  ...
                  <td className="px-3 py-2">
                    <button onClick={() => onSelect(karyawan)} className="bg-blue-600 hover:bg-blue-700 text-white p-2 rounded text-xs font-medium transition-colors flex items-center justify-center" title="Lihat detail karyawan">
      ...
      <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
        <div className="flex items-start">
          ...
          <div>
            ...
            <p className="text-blue-700 text-xs"><strong>AK Real = AK Awal + AK Tambahan</strong>. AK Tambahan dihitung otomatis sejak TMT Jabatan sampai hari ini dengan asumsi predikat kinerja "Baik".</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="bg-white rounded-xl p-4 shadow-lg border border-gray-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
        <div className="space-y-2">
          ...
          <input type="month" value={formData.periode} onChange={(e) => setFormData({...formData, periode: e.target.value})} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
        <div className="space-y-2">
          <label className="text-sm font-medium text-gray-700">Jenis Penilaian</label>
          <select value={formData.jenisPenilaian} onChange={(e) => setFormData({ ...formData, jenisPenilaian: e.target.value as 'Tahunan' | 'Periodik' })} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        {formData.jenisPenilaian === 'Periodik' && (
          <div className="space-y-2">
            ...
            <input type="number" min="1" max="11" value={formData.bulanPeriodik} onChange={(e) => setFormData({...formData, bulanPeriodik: parseInt(e.target.value)})} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
        )}
      </div>
      ...
      <div className="space-y-2 mb-4">
        ...
        <input type="text" value={formData.keterangan} onChange={(e) => setFormData({...formData, keterangan: e.target.value})} placeholder="Capaian SKP, evaluasi, dll." className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
      </div>
      <div className="bg-gray-50 rounded-lg p-3 mb-4">
        <h4 className="font-semibold text-gray-700 mb-2 text-sm">Perhitungan Angka Kredit:</h4>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
          <div><p className="text-gray-600">Jabatan</p><p className="font-semibold text-gray-800">{karyawan.jabatan}</p></div>
          <div><p className="text-gray-600">Koefisien</p><p className="font-semibold text-gray-800">{currentKoefisien}</p></div>
          <div><p className="text-gray-600">Predikat</p><p className="font-semibold text-gray-800">{formData.predikatKinerja * 100}%</p></div>
          <div><p className="text-gray-600">Hasil AK</p><p className="font-bold text-blue-600">{calculatedAK}</p></div>
        </div>
      </div>
      <button onClick={handleCalculate} className="w-full bg-blue-600 text-white py-2 px-4 rounded-lg font-semibold hover:bg-blue-700 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 text-sm">
        Simpan Data Kinerja
      </button>
    </div>
  );
...
const EmployeeDashboard: React.FC<{ karyawan: Karyawan }> = ({ karyawan }) => {
  ...
  const isJenjangPertama = karyawan.jabatan.includes('Pertama') && karyawan.golongan === 'III/b';
  const kebutuhanPangkatUntukProgress = isJenjangPertama ? estimasi.kebutuhanAKJabatan : estimasi.kebutuhanAKPangkat;
  const penjelasanPangkatUntukProgress = isJenjangPertama ? "Kebutuhan 100 AK kumulatif untuk naik jenjang ke Ahli Muda" : `Kebutuhan ${estimasi.kebutuhanAKPangkat} AK untuk kenaikan pangkat ke ${estimasi.golonganBerikutnya}`;
  const kekuranganPangkatUntukProgress = Math.max(0, kebutuhanPangkatUntukProgress - estimasi.akRealSaatIni);
  const kekuranganJabatanUntukProgress = Math.max(0, estimasi.kebutuhanAKJabatan - estimasi.akRealSaatIni);
  const penjelasanKebutuhan = AngkaKreditCalculator.getPenjelasanKebutuhan(karyawan.jabatan, karyawan.kategori);
    ...
    <div className="space-y-4">
      ...
      {rekomendasiKarir && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
          <div className="flex items-start">
            <span className="text-xl mr-3">Lightbulb</span>
            <div>
      ...
      <div className="bg-white rounded-xl p-4 shadow-lg border border-gray-200">
        <div className="mb-4 pb-3 border-b border-gray-200">
          <div className="mt-2 p-2 bg-blue-50 rounded-lg">
            ...
            <p className="text-xs text-gray-600">(Database: {karyawan.akKumulatif.toFixed(2)} + Tambahan: {estimasi.akTambahan.toFixed(2)})</p>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <ProgressBar
            label={isJenjangPertama ? `Kenaikan Jenjang ke ${estimasi.jabatanBerikutnya}` : `Kenaikan Pangkat ke ${estimasi.golonganBerikutnya}`}
            akSaatIni={karyawan.akKumulatif}
            akRealSaatIni={estimasi.akRealSaatIni}
            kebutuhanAK={kebutuhanPangkatUntukProgress}
            type="pangkat"
            bulanDibutuhkan={kekuranganPangkatUntukProgress > 0 ? Math.ceil(kekuranganPangkatUntukProgress / estimasi.akPerBulan) : 0}
          ...
          <ProgressBar
            ...
            type="jabatan"
            bulanDibutuhkan={kekuranganJabatanUntukProgress > 0 ? Math.ceil(kekuranganJabatanUntukProgress / estimasi.akPerBulan) : 0}
            akTambahan={estimasi.akTambahan}
        ...
        {!selectedKaryawan ? (
          ...
          <>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-3">
              <div>
                <button onClick={() => setSelectedKaryawan(null)} className="flex items-center text-blue-600 hover:text-blue-800 font-medium mb-1 text-sm"><span className="mr-1">Back</span> Kembali ke Daftar Karyawan</button>
