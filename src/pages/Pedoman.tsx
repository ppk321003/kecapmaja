// App.tsx - VERSI LENGKAP, SEDERHANA, EFISIEN, TANPA DROPDOWN PREDIKAT
...
class AngkaKreditCalculator {
  static getKoefisien(jenjangJabatan: string): number {
    ...
    return map[jenjangJabatan] || 12.5;
  }
...
// Tabel Daftar Karyawan (Ringkas)
const DaftarKaryawan: React.FC<{ list: Karyawan[]; onSelect: (k: Karyawan) => void; selectedNip: string | null }> = ({ list, onSelect, selectedNip }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow border overflow-hidden mb-6">
      <div className="p-4 border-b">
        <input 
          type="text" placeholder="Cari nama, NIP, unit..." 
        ...
        <table className="w-full text-xs">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-3 py-2 text-center">AK</th>
          ...
          <tbody>
            {filtered.map(k => (
              <tr 
                key={k.nip} 
                onClick={() => onSelect(k)} 
                className={`border-t cursor-pointer hover:bg-blue-50 ${selectedNip === k.nip ? 'bg-blue-100' : ''}`}
              >
                ...
                <td className="px-3 py-2 text-center font-bold text-blue-600">{k.akKumulatif.toFixed(1)}</td>
