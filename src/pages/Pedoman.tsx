      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="space-y-2">
          ...
          <div>
            ...
            <p className="font-medium text-gray-700">{karyawan.kategori}</p>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div>
            ...
            <p className="font-medium text-gray-700">{masaKerja.years} tahun {masaKerja.months} bulan</p>
          </div>
        </div>
...
// Fungsi untuk menentukan warna berdasarkan jabatan
const getJabatanColorClass = (jabatan: string): string => {
  if (jabatan.includes('Ahli Pertama')) return 'bg-purple-100 text-purple-800';
  if (jabatan.includes('Ahli Muda')) return 'bg-indigo-100 text-indigo-800';
  if (jabatan.includes('Ahli Madya')) return 'bg-blue-100 text-blue-800';
  if (jabatan.includes('Ahli Utama')) return 'bg-teal-100 text-teal-800';
  if (jabatan.includes('Terampil')) return 'bg-orange-100 text-orange-800';
  if (jabatan.includes('Mahir')) return 'bg-yellow-100 text-yellow-800';
  if (jabatan.includes('Penyelia')) return 'bg-red-100 text-red-800';
  if (jabatan.includes('Kepala')) return 'bg-gray-100 text-gray-800';
...
const PredikatKinerjaRadio: React.FC<{
  ...
  const predikatOptions = [
    { value: 1.50, label: 'Sangat Baik', color: 'from-green-500 to-green-600', description: '150% - Performa luar biasa' },
    { value: 1.00, label: 'Baik', color: 'from-blue-500 to-blue-600', description: '100% - Performa standar' },
    { value: 0.75, label: 'Cukup', color: 'from-yellow-500 to-yellow-600', description: '75% - Perlu peningkatan' },
    { value: 0.50, label: 'Kurang', color: 'from-red-500 to-red-600', description: '50% - Perlu perbaikan serius' }
  ];
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded-full text-xs ${
                      getJabatanColorClass(karyawan.jabatan)
                    }`}>
                      {karyawan.jabatan}
                    </span>
                  </td>
...
// Komponen Estimasi Kenaikan dengan Radio Button
