...
// Helper function to format data for spreadsheet
const formatDataForSpreadsheet = (data: FormValues, organikGroups: PersonGroup[], mitraGroups: PersonGroup[], programs: any[], kegiatanList: any[], kroList: any[], roList: any[], komponenList: any[], akunList: any[], organikList: any[], mitraList: any[]) => {
  // Get names for IDs
  const getProgramName = (id: string) => programs.find(p => p.id === id)?.name || "";
  const getKegiatanName = (id: string) => kegiatanList.find(k => k.id === id)?.name || "";
  const getKROName = (id: string) => kroList.find(k => k.id === id)?.name || "";
  const getROName = (id: string) => roList.find(r => r.id === id)?.name || "";
  const getKomponenName = (id: string) => komponenList.find(k => k.id === id)?.name || "";
  const getAkunName = (id: string) => akunList.find(a => a.id === id)?.name || "";
  const getOrganikName = (id: string) => organikList.find(o => o.id === id)?.name || "";
  const getMitraName = (id: string) => mitraList.find(m => m.id === id)?.name || "";
  const getPembuatDaftarName = (id: string) => organikList.find(o => o.id === id)?.name || "";
  ...
  // Combine all transport details
  const allTransportDetails = [
    ...organikGroups.flatMap(group => 
  ...
  // Calculate total
  const total = allTransportDetails.reduce((sum, item) => sum + (parseInt(item.rate) || 0), 0);

  // Prepare data for spreadsheet - one row per transport detail
  const rows = allTransportDetails.map((detail, index) => {
    const baseData = {
      // No - will be auto-generated by spreadsheet
      ...
      Program: getProgramName(data.program),
      Kegiatan: getKegiatanName(data.kegiatan),
      KRO: getKROName(data.kro),
      RO: getROName(data.ro),
      Komponen: getKomponenName(data.komponen),
      Akun: getAkunName(data.akun),
      "Tanggal Pengajuan": formatDateForSheet(data.tanggalSpj),
      "Pembuat daftar": getPembuatDaftarName(data.pembuatDaftar),
      Total: total
    };
    ...
    if (detail.type === "organik") {
      return {
        ...baseData,
        Organik: detail.personName,
        "Dari Kecamatan (organik)": detail.dariKecamatan,
        "Kecamatan Tujuan (organik)": detail.kecamatanTujuan,
        "rate (organik)": parseInt(detail.rate) || 0,
        "tanggal (organik)": formatDateForSheet(detail.tanggalPelaksanaan),
        "Mitra Statistik": "",
        "Dari Kecamatan (mitra)": "",
        "Kecamatan Tujuan (mitra)": "",
        "rate (mitra)": "",
        "tanggal (mitra)": ""
      };
    } else {
      return {
        ...baseData,
        Organik: "",
        "Dari Kecamatan (organik)": "",
        "Kecamatan Tujuan (organik)": "",
        "rate (organik)": "",
        "tanggal (organik)": "",
        "Mitra Statistik": detail.personName,
        "Dari Kecamatan (mitra)": detail.dariKecamatan,
        "Kecamatan Tujuan (mitra)": detail.kecamatanTujuan,
        "rate (mitra)": parseInt(detail.rate) || 0,
        "tanggal (mitra)": formatDateForSheet(detail.tanggalPelaksanaan)
      };
    }
  });
...
const KuitansiTransportLokal = () => {
  ...
  // Setup submission dengan spreadsheet ID yang benar
  const submitMutation = useSubmitToSheets({
    ...
    onSuccess: () => {
      toast({ title: "Berhasil!", description: "Data berhasil disimpan ke spreadsheet" });
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      const spreadsheetData = formatDataForSpreadsheet(
        data, 
        organikGroups, 
        mitraGroups, 
        programs, 
      ...
      // Submit each row to spreadsheet
      for (const rowData of spreadsheetData) {
        await submitMutation.mutateAsync(rowData);
      }
