...
// Komponen MultiSelect sederhana
...
const MultiSelect = ({ options, value = [], onChange, placeholder }: MultiSelectProps) => {
  ...
  return (
    <div className="space-y-2">
      <Select value="" onValueChange={toggleValue}>
        <SelectTrigger>
          ...
          {options.map(option => (
            <SelectItem key={option.value} value={option.value}>
              <div className="flex items-center">
                {value.includes(option.value) && <Check className="h-4 w-4 mr-2" />}
                {option.label}
              </div>
            </SelectItem>
          ))}
      ...
      {value.length > 0 && (
        <div className="mt-2 p-2 bg-muted rounded-md">
          <p className="text-sm text-muted-foreground">
            Terpilih: {value.map(val => options.find(o => o.value === val)?.label).join(', ')}
          </p>
        </div>
      )}
    </div>
