...
export default function AkiToBendahara() {
  ...
  // Data dummy untuk testing - ganti dengan data real nanti
  const dummyData: DataRow[] = [
    {
      no: 1,
      bulan: "Januari",
      tahun: 2024,
      namaPetugas: "Ahmad Susanto",
      namaBank: "BCA",
      noRekening: "1234567890",
      jumlah: 2500000,
      "Sensus Pertanian": 1500000,
      "Survei Sosial": 1000000,
    },
    {
      no: 2,
      bulan: "Januari",
      ...
      jumlah: 3000000,
      "Sensus Pertanian": 2000000,
      "Pengolahan Data": 1000000,
    },
    {
      no: 3,
      bulan: "Februari",
      ...
      jumlah: 2750000,
      "Survei Sosial": 1750000,
      "Pelatihan Mitra": 1000000,
    },
    {
      no: 4,
      bulan: "Februari",
      tahun: 2024,
      namaPetugas: "Maya Sari",
      namaBank: "Mandiri",
      noRekening: "5566778899",
      jumlah: 3200000,
      "Sensus Pertanian": 1200000,
      "Pengolahan Data": 1500000,
      "Koordinasi Lapangan": 500000,
    }
  ];

  // Fetch data - menggunakan dummy data dulu
  const fetchData = async () => {
    try {
      ...
      // Untuk production, gunakan kode ini:
      /*
      ...
      */
      ...
      setAvailableKegiatan(["Sensus Pertanian", "Survei Sosial", "Pengolahan Data", "Pelatihan Mitra", "Koordinasi Lapangan"]);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  // Filter data berdasarkan search, bulan, tahun
  useEffect(() => {
    let result = data;
  ...
  const calculateTotals = () => {
    ...
    getDisplayedColumns().forEach(column => {
                        ...
                        return (
                          <TableHead 
                            ...
                            className={`
                              ...
                              ${column === 'no' ? 'w-16' : ''}
                              ${column === 'namaPetugas' ? 'min-w-48' : ''}
                              ${column === 'namaBank' ? 'min-w-32' : ''}
                              ${column === 'noRekening' ? 'min-w-40' : ''}
                              ${availableKegiatan.includes(column) ? 'text-right min-w-40' : ''}
                            `}
                            style={
                              isSticky ? { 
                                left: index === 0 ? 0 : 
                                  index === 1 ? '4rem' : 
                                  index === 2 ? 'calc(4rem + 12rem)' : 
                                  'calc(4rem + 12rem + 8rem)'
                              } : {}
                            }
                          >
                            {column === 'no' ? 'No' : 
