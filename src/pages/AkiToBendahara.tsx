...
export default function AkiToBendahara() {
  ...
  // Update available kegiatan dan filter data
  useEffect(() => {
    let result = data;
    ...
    if (result.length > 0) {
      // Dapatkan semua kegiatan yang ada di data terfilter
      const allKegiatanInFilteredData = new Set<string>();
      
      result.forEach(item => {
        Object.keys(item).forEach(key => {
          if (!baseColumns.includes(key)) {
            allKegiatanInFilteredData.add(key);
          }
        });
      });

      // Filter hanya kegiatan yang memiliki nilai > 0 pada data terfilter
      const kegiatanWithValues: string[] = [];
      
      allKegiatanInFilteredData.forEach(kegiatan => {
        const hasNonZeroValue = result.some(item => {
          const value = Number(item[kegiatan]);
          return value > 0;
        });
  ...
  }, [searchTerm, selectedBulan, selectedTahun, data, selectedKegiatan]);
  ...
  // Format number tanpa currency symbol dan tanpa pemisah ribuan
  ...
  const hasDataToShow = filteredData.length > 0;
            ...
            <Select value={selectedKegiatan} onValueChange={setSelectedKegiatan}>
              <SelectTrigger>
                <SelectValue placeholder={
                  availableKegiatan.length === 0 && (selectedBulan || selectedTahun) 
              ...
              <SelectContent>
                {availableKegiatan.length === 0 ? (
                  <SelectItem value="" disabled>
                    {selectedBulan || selectedTahun ? "Tidak ada kegiatan tersedia" : "Pilih bulan/tahun terlebih dahulu"}
                  </SelectItem>
                ) : (
          ...
          {isLoading ? (
            ...
            <div className="flex items-center justify-center h-64 bg-muted rounded-lg">
              <div className="text-center text-muted-foreground">
                ...
                <p className="text-sm mt-2">Pilih filter terlebih dahulu untuk melihat data rekap honor</p>
              </div>
            </div>
          ) : (
                          ...
                          return (
                            <TableCell 
                              ...
                              className={`
                                ...
                                ${column === 'namaBank' ? 'w-[80px] truncate' : ''}
                              `}
                              style={
                                isSticky ? { 
                            ...
                            >
                              {column === 'jumlah' || column === selectedKegiatan
                                ...
                                : column === 'namaBank' 
                                  ? (row[column] as string).substring(0, 8)
                                  : row[column]
                              }
                            </TableCell>
                          );
