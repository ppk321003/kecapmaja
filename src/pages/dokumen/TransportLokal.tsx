            ...
            <Card>
              <CardContent className="p-6 space-y-8">
                ...
                <div className="space-y-6">
                      ...
                      addTransportDetail(type as 'organik' | 'mitra', id);
                        ...
                        <SelectContent>
                          ...
                          {availableOrganik.length > 0 && <>
                              ...
                              {availableOrganik.map(org => <SelectItem key={org.id} value={`organik|${org.id}`}>
                                  {org.name}
                                </SelectItem>)}
                            </>}
                          {availableMitra.length > 0 && <>
                              ...
                              {availableMitra.map(mitra => <SelectItem key={mitra.id} value={`mitra|${mitra.id}`}>
                  ...
                  {transportDetails.length > 0 ? <div className="space-y-4">
                      {transportDetails.map((detail, index) => <Card key={detail.id} className="overflow-hidden">
                          <CardContent className="p-4">
                            <div className="flex justify-between items-center mb-4">
                              ...
                              <Button variant="ghost" size="icon" onClick={() => removeTransportDetail(detail.id)}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                              <div className="space-y-2">
                                ...
                                <Input type="number" value={detail.banyaknya} onChange={e => handleTransportItemChange(detail.id, "banyaknya", parseInt(e.target.value, 10) || 0)} placeholder="0" />
                              </div>
                            </div>
                            ...
                            <div className="space-y-2 mb-4">
                              ...
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                                {kecamatanOptions.map(kec => <div key={kec} className="flex items-center space-x-2">
                                    <Checkbox id={`kec-${detail.id}-${kec}`} checked={detail.kecamatanTujuan.includes(kec)} onCheckedChange={checked => handleKecamatanChange(detail.id, kec, !!checked)} />
                            ...
                            {detail.kecamatanTujuan.length > 0 && <div className="space-y-3 mb-4">
                                <Label>Rate Transport per Kecamatan (Rp)</Label>
                                <div className="space-y-2">
                                  {detail.kecamatanTujuan.map((kec, kecIndex) => <div key={kecIndex} className="grid grid-cols-2 gap-2 items-center">
                                      <Label className="text-sm">{kec}</Label>
                                      <Input type="number" value={detail.rateTranslok[kecIndex] || 0} onChange={e => handleRateChange(detail.id, kecIndex, e.target.value)} placeholder="0" />
                                    </div>)}
                                </div>
                              </div>}
                            ...
                            <div>
                              ...
                              <Input value={detail.jumlah.toLocaleString()} disabled className="font-bold" />
                            </div>
                          </CardContent>
                        </Card>)}
                      ...
                      <div className="flex justify-end">
                        <div className="w-full md:w-1/2 lg:w-1/3 space-y-2">
                          ...
                          <Input value={totalAmount.toLocaleString()} disabled className="text-lg font-bold" />
                        </div>
                      </div>
