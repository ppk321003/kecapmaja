...
export function FormPengajuanPerubahan({ open, onOpenChange, onSubmit }: Props) {
  ...
  // Calculate current installment and minimum
  const cicilanInfo = useMemo(() => {
    ...
    const currentCicilan = memberInfo.limit.cicilanPokok || minCicilan;
  ...
  const cicilanValidation = useMemo(() => {
    ...
    if (saldoPiutang <= 0) {
      return { valid: true, warnings: ['Tidak ada hutang yang perlu dicicil'] };
    }
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
            ...
            {/* Cicilan Info Box */}
            {watchJenisPerubahan === 'Cicilan' && memberInfo && cicilanInfo.saldoPiutang > 0 && (
              <div className="p-4 rounded-lg bg-primary/5 border border-primary/20 space-y-4">
                <div className="flex items-center gap-2">
                  <Calculator className="h-5 w-5 text-primary" />
                  <h4 className="font-semibold">Informasi Cicilan</h4>
                </div>
            ...
            {/* Current vs New Value Display for non-Cicilan */}
            {memberInfo && watchJenisPerubahan !== 'Cicilan' && (
              <div className="p-4 rounded-lg bg-muted/30 border">
                <p className="text-sm text-muted-foreground mb-3">Perubahan Nilai</p>
                <div className="flex items-center justify-center gap-4">
                  <div className="text-center">
                  ...
                  <div className="text-center">
                    ...
                    <p className={`text-xl font-bold ${isIncrease ? 'text-green-600' : 'text-destructive'}`}>
            ...
            {watchJenisPerubahan === 'Cicilan' && cicilanValidation.warnings.length > 0 && (
              <Alert className={`${!cicilanValidation.valid ? 'bg-destructive/10 border-destructive/30' : 'bg-yellow-50 border-yellow-200'}`}>
                <AlertTriangle className={`h-4 w-4 ${!cicilanValidation.valid ? 'text-destructive' : 'text-yellow-600'}`} />
                <AlertDescription className={!cicilanValidation.valid ? 'text-destructive' : 'text-yellow-700'}>
            ...
            <DialogFooter>
              ...
              <Button 
                type="submit" 
                disabled={!cicilanValidation.valid || !memberInfo || watchNilaiBaru === 0}
              >
                Ajukan Perubahan
              </Button>
            </DialogFooter>
