...
export const AkunSelect = ({ value, onValueChange, onChange, disabled }: AkunSelectProps) => {
  ...
  const mapedData: AkunOption[] = data.map((item, index) => ({
  ...
  const filteredOptions = useMemo(() => {
    ...
    return mapedData.filter(option =>
      option.akun.toLowerCase().includes(searchTerm.toLowerCase()) ||
      option.kode.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [mapedData, searchTerm]);

  const selectedAkun = mapedData.find(option => option.id === value);
      ...
      >
        <div className="flex items-center gap-2 overflow-hidden">
          {loading ? (
            ...
            <span className="truncate">{selectedAkun.kode} - {selectedAkun.akun}</span>
          ) : (
