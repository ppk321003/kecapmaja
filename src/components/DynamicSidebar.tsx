...
const DynamicSidebar: React.FC<DynamicSidebarProps> = ({
  ...
  const groupedMenuItems = {
    ...
    bahanRevisi: menuItems.filter(item => item.category === "bahan-revisi"),
    dokumen: menuItems.filter(item => item.category === "dokumen"),
    laporan: menuItems.filter(item => item.category === "laporan"),
    referensi: menuItems.filter(item => item.category === "referensi"),
    inventaris: menuItems.filter(item => item.category === "inventaris")
  };
        ...
        <div className="flex h-full flex-col">
          <div className="flex-1 overflow-y-auto p-4">
            <div className="space-y-2">
              {[...Array(5)].map((_, i) => (
                <div key={i} className="h-8 w-full animate-pulse rounded-lg bg-sidebar-accent/30"></div>
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="flex h-full flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4 bg-teal-700">
          <nav className="space-y-4">
            {/* Menu Utama */}
            <div>
              <h3 className="px-3 py-2 text-xs font-semibold uppercase tracking-wider text-sidebar-foreground/70">Menu Utama</h3>
              <div className="space-y-1">
                {groupedMenuItems.main.map((item: SidebarItem) => (
                  <Link
                    key={item.id}
                    to={item.path}
            ...
            {/* Bahan Revisi 3210 */}
            <div>
              <h3 className="px-3 py-2 text-xs font-semibold uppercase tracking-wider text-sidebar-foreground/70">Bahan Revisi 3210</h3>
              <div className="space-y-1">
                {groupedMenuItems.bahanRevisi.map((item: SidebarItem) => (
                  <Link
                    key={item.id}
                    to={item.path}
                    className={`flex items-center space-x-2 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
            ...
            <div>
              <h3 className="px-3 py-2 text-xs font-semibold uppercase tracking-wider text-sidebar-foreground/70">Dokumen dan Formulir</h3>
              <div className="space-y-1">
                {groupedMenuItems.dokumen.map((item: SidebarItem) => (
                  <Link
                    key={item.id}
                    to={item.path}
                    className={`flex items-center space-x-2 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
            ...
            {/* Laporan & Rekap */}
            <div>
              <h3 className="px-3 py-2 text-xs font-semibold uppercase tracking-wider text-sidebar-foreground/70">Laporan & Rekap</h3>
              <div className="space-y-1">
                {groupedMenuItems.laporan.map((item: SidebarItem) => (
                  <Link
                    key={item.id}
                    to={item.path}
                    className={`flex items-center space-x-2 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
            ...
            {/* Referensi & Peraturan */}
            <div>
              <h3 className="px-3 py-2 text-xs font-semibold uppercase tracking-wider text-sidebar-foreground/70">Referensi & Peraturan</h3>
              <div className="space-y-1">
                {groupedMenuItems.referensi.map((item: SidebarItem) => (
                  <Link
                    key={item.id}
                    to={item.path}
                    className={`flex items-center space-x-2 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
            ...
            {/* Inventaris & Arsip */}
            <div>
              <h3 className="px-3 py-2 text-xs font-semibold uppercase tracking-wider text-sidebar-foreground/70">Inventaris & Arsip</h3>
              <div className="space-y-1">
                {groupedMenuItems.inventaris.map((item: SidebarItem) => (
                  <Link
                    key={item.id}
                    to={item.path}
                    className={`flex items-center space-x-2 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
        ...
        <div className="border-t border-sidebar-border p-4 bg-teal-700">
          ...
          <Button variant="outline" onClick={toggleTheme} className="w-full text-sidebar-foreground border-sidebar-border hover:text-sidebar-foreground rounded-none font-medium text-sm bg-emerald-900 hover:bg-emerald-800">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mr-2 h-4 w-4 dark:hidden">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mr-2 h-4 w-4 hidden dark:inline">
