...
const DynamicSidebar: React.FC<DynamicSidebarProps> = ({
  ...
  const groupedMenuItems = menuItems.reduce((acc, item) => {
    ...
    if (["Buat Dokumen", "Download Dokumen", "Blanko Visum", "Surat Pernyataan"].includes(item.title)) {
      group = "Dokumen dan Formulir";
    } else if (["Rekap Honor Mitra", "Rekap SPK dan BAST", "Riwayat Kertas Kerja"].includes(item.title)) {
      group = "Laporan & Rekap";
    } else if (["Perka BPS", "SBM 2025", "SK Transport Lokal"].includes(item.title)) {
      group = "Referensi & Peraturan";
    } else if (["Stock Opname", "Kecap Maja (OLD)"].includes(item.title)) {
      group = "Inventaris & Arsip";
    } else if (["Bahan Revisi 3210"].includes(item.title)) {
                ...
                <div key={groupName} className="space-y-1">
                  ...
                  >
                    ...
                    <svg
                      ...
                      className={`h-4 w-4 transition-transform ${expandedGroups[groupName] ? "rotate-90" : ""}`}
                    >
