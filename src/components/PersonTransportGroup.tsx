...
const PersonTransportGroup = memo(({
  ...
  return <div className="border rounded-lg p-4 mb-4 bg-card">
      <div className="flex justify-between items-center mb-4">
        <h4 className="text-base font-semibold text-primary">
          {type === "organik" ? "Organik BPS" : "Mitra Statistik"}
        </h4>
        <Button type="button" variant="outline" size="sm" onClick={onRemovePerson}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 p-4 bg-muted/30 rounded-lg">
        <div className="space-y-2">
          ...
          <FormSelect placeholder="Pilih nama" options={personList.map(person => ({
          value: person.id,
          label: person.name
        }))} value={personId} onChange={onUpdatePerson} />
        </div>
        ...
        <div className="space-y-2">
          ...
          <Select value={dariKecamatan} onValueChange={onUpdateDariKecamatan}>
            ...
            <SelectContent>
              {kecamatanList.map(kecamatan => <SelectItem key={kecamatan} value={kecamatan}>
                  {kecamatan}
                </SelectItem>)}
            </SelectContent>
          </Select>
      ...
      <div className="space-y-3">
        ...
        {trips.map((trip, tripIndex) => <div key={tripIndex} className="border rounded-md p-3 bg-background">
            <div className="flex justify-between items-center mb-3">
              <span className="text-sm font-medium text-muted-foreground">
                Perjalanan #{tripIndex + 1}
              </span>
              {trips.length > 1 && <Button type="button" variant="ghost" size="sm" onClick={() => onRemoveTrip(tripIndex)}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div className="space-y-2">
                ...
                <Select value={trip.kecamatanTujuan} onValueChange={value => onUpdateTrip(tripIndex, "kecamatanTujuan", value)}>
              ...
              <div className="space-y-2">
                ...
                <Input type="text" pattern="[0-9]*" value={trip.rate || ""} onChange={e => {
              ...
              <div className="space-y-2">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" className={cn("w-full pl-3 text-left font-normal text-sm", !trip.tanggalPelaksanaan && "text-muted-foreground")}>
                      {trip.tanggalPelaksanaan ? format(trip.tanggalPelaksanaan, "dd/MM/yyyy", {
                    locale: idLocale
                  ...
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar mode="single" selected={trip.tanggalPelaksanaan} onSelect={date => onUpdateTrip(tripIndex, "tanggalPelaksanaan", date)} disabled={date => false} initialFocus />
                  </PopoverContent>
                </Popover>
              </div>
            </div>
