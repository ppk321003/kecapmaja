...
start = text.find('const exportBAPPToDocx = async (pmlRow: any, pplUnderPML: AggregatedData[]) => {')
...
new = '''const exportBAPPToDocx = async (
  ...
  const getEmail = (row: any) =>
    String(getColumnValue(row, "email", ["email", "Email", "email_ppl", "email_pml", "Email PPL", "Email PML"], "")).trim().toLowerCase();
  ...
  (usersData || []).forEach((u: any) => {
    ...
    const slsRaw = String(getColumnValue(u, "sls", ["__col_7", "H", "h", "sls", "regioncode", "region_code"], "")).trim();
    if (slsRaw) {
      const parts = slsRaw.split(/[,:;\\s]+/).map(normalizeCode).filter(Boolean);
      if (parts.length > 0) {
        usersSlsByEmail.set(email, Array.from(new Set([...(usersSlsByEmail.get(email) || []), ...parts])));
      }
    }
  ...
  (qualityKeluargaData || []).forEach((row: any) => {
    const code = normalizeCode(getColumnValue(row, "idsubsls_25_2", ["idsubsls_25_2", "idsubsls", "idsubsls252", "kdsubsls", "kdsls", "kode", "code", "__col_0", "__col_3"], ""));
    if (!code) return;
    const keluargaCount = parseNumber(getColumnValue(row, "total_hasil_pendataan", ["total_hasil_pendataan", "total hasil pendataan", "totalhasilpendataan", "jumlah_keluarga", "jumlah keluarga"], "0"));
  ...
  (qualityUsahaRumahData || []).forEach((row: any) => {
    const code = normalizeCode(getColumnValue(row, "idsubsls_25_2", ["idsubsls_25_2", "idsubsls", "idsubsls252", "kdsubsls", "kdsls", "kode", "code", "__col_0", "__col_3"], ""));
    if (!code) return;
    const usahaDalamKeluarga = parseNumber(getColumnValue(row, "jumlah_usaha_dlm_keluarga", ["jumlah usaha dalam keluarga yang berhasil didata", "jumlah_usaha_dlm_keluarga", "usaha dalam keluarga didata", "usaha_dlm_keluarga", "jumlah_usaha_dalam_keluarga", "usaha_dlm_rumah", "__col_3", "D"], "0"));
  ...
  (qualityUsahaData || []).forEach((row: any) => {
    const code = normalizeCode(getColumnValue(row, "idsubsls_25_2", ["idsubsls_25_2", "idsubsls", "idsubsls252", "kdsubsls", "kdsls", "kode", "code", "__col_0", "__col_3"], ""));
    if (!code) return;
    const totalUsaha = parseNumber(getColumnValue(row, "total", ["total", "total_usaha", "total usaha", "jumlah_usaha_total", "jumlah usaha total", "Total Usaha"], "0"));
  ...
  const tableRows: DocxTableRow[] = [
    new DocxTableRow({
      children: [
        new DocxTableCell({ width: { size: 900, type: WidthType.DXA }, children: [getDocumentCell("No")], verticalAlign: VerticalAlign.CENTER }),
        new DocxTableCell({ width: { size: 4200, type: WidthType.DXA }, children: [getDocumentCell("Nama Petugas Lapangan Sensus")], verticalAlign: VerticalAlign.CENTER }),
        new DocxTableCell({ width: { size: 2800, type: WidthType.DXA }, children: [getDocumentCell("Kecamatan")], verticalAlign: VerticalAlign.CENTER }),
        new DocxTableCell({ width: { size: 2800, type: WidthType.DXA }, children: [getDocumentCell("Desa/Lurah")], verticalAlign: VerticalAlign.CENTER }),
        new DocxTableCell({ width: { size: 2400, type: WidthType.DXA }, children: [getDocumentCell("SLS/Sub-SLS")], verticalAlign: VerticalAlign.CENTER }),
        new DocxTableCell({ width: { size: 2200, type: WidthType.DXA }, children: [getDocumentCell("Muatan Hasil Pendataan")], verticalAlign: VerticalAlign.CENTER }),
        new DocxTableCell({ width: { size: 2200, type: WidthType.DXA }, children: [getDocumentCell("Keluarga")], verticalAlign: VerticalAlign.CENTER }),
        new DocxTableCell({ width: { size: 2200, type: WidthType.DXA }, children: [getDocumentCell("Usaha")], verticalAlign: VerticalAlign.CENTER }),
      ],
    }),
  ];
  ...
  if (pplUnderPML.length === 0) {
    ...
    pplUnderPML.forEach((ppl) => {
      const emailPPL = String(getColumnValue(ppl, "email_ppl", ["email_ppl", "email", "Email", "email_ppl", "Email PPL"], "")).trim().toLowerCase();
    ...
    sections: [
      {
        ...
        children: [
          ...
          new Paragraph({ children: [new TextRun({ text: f"PML: {getDocString(pmlRow.nama_pml)}", size: 20, bold: True })] }),
          new Paragraph({ children: [new TextRun({ text: f"Kecamatan: {getDocString(pmlRow.kecamatan)}", size: 20 })] }),
          new Paragraph({ children: [new TextRun({ text: "", size: 20 })] }),
